...
import {
  CodeType,
  ICode,
} from "./model-gen/org.accordproject.templatemark@0.5.0";
...
function checkCode(code: ICode) {
  if (code.type !== CodeType.ES_2020) {
    throw new Error(
      `Cannot run ${code.contents} as it is not ES_2020 JavaScript.`,
    );
  }
}
...
const javaScriptEvaluator = isBrowser
  ? new JavaScriptEvaluator()
  : new JavaScriptEvaluator({
      maxWorkers: process.env.MAX_WORKERS
        ? Number.parseInt(process.env.MAX_WORKERS)
        : os.availableParallelism(), // how many child processes
      waitInterval: process.env.WAIT_INTERVAL
        ? Number.parseInt(process.env.WAIT_INTERVAL)
        : 50, // how long to wait before rescheduling work
      maxQueueDepth: process.env.MAX_QUEUE_DEPTH
        ? Number.parseInt(process.env.MAX_QUEUE_DEPTH)
        : 1000, // max requests to queue
    });
...
async function evaluateJavaScript(
  ...
  try {
    const request = {
      code: expression,
      argumentNames: functionArgNames,
      arguments: functionArgValues,
    };
    if (options?.childProcessJavaScriptEvaluation) {
      if (isBrowser) {
        throw new Error(
          "Child process evaluation is not supported inside web browser",
        );
      }
      const evalOptions = options?.timeout
        ? { timeout: options.timeout }
        : undefined;
      const r = await javaScriptEvaluator.evalChildProcess(
        request,
        evalOptions,
      );
      return r;
    ...
    throw new Error(
      `Caught error ${JSON.stringify(
        err,
      )} evaluating ${expression} with arguments ${JSON.stringify(
        functionArgValues,
      )}`,
    );
  }
    ...
    if (
      ...
      if (FORMULA_DEFINITION_RE.test(nodeClass)) {
        if (context.code) {
          ...
          const evalResponse = await evaluateJavaScript(
            clauseLibrary,
            data,
            context.code.contents,
            options,
          );
        ...
        CONDITIONAL_DEFINITION_RE.test(nodeClass) ||
        CLAUSE_DEFINITION_RE.test(nodeClass)
      ) {
 ...
 */
async function generateOptionalBlocks(
  modelManager: ModelManager,
  clauseLibrary: object,
  templateMark: object,
  data: TemplateData,
  options?: GenerationOptions,
): Promise<OptionalBlockResult> {
      ...
      if (OPTIONAL_DEFINITION_RE.test(nodeClass)) {
        ...
        if (variableValues.length > 0) {
          ...
          if (context.whenSome && context.whenSome.length > 0) {
            ...
            const subResult = await generateAgreement(
              modelManager,
              clauseLibrary,
              whenSomeParagraph,
 ...
 */
async function generateRecursiveBlocks(
  modelManager: ModelManager,
  clauseLibrary: object,
  templateMark: object,
  data: TemplateData,
  nodeRegExp: RegExp,
  childNodeClass: string,
  options?: GenerationOptions,
  ...
  for (let n = 0; n < paths.length; n++) {
    ...
    if (
      ...
      if (nodeRegExp.test(nodeClass)) {
        ...
        if (variableValues.length === 0) {
          throw new Error(
            `No values found for path '${path}' in data ${JSON.stringify(
              data,
            )}.`,
          );
        } else {
          ...
          if (!Array.isArray(arrayData)) {
            throw new Error(
              `Values found for path '${path}' in data ${data} is not an array: ${arrayData}.`,
            );
          } else {
            const nodes = [];
...
// 👇 This function already exists below
async function generateAgreement(
  ...
  const userCodeResults = await evaluateUserCode(
    clauseLibrary,
    templateMark,
    data,
    options,
  ...
  const listBlockResults = await generateRecursiveBlocks(
    modelManager,
    clauseLibrary,
    templateMark,
    data,
    LISTBLOCK_DEFINITION_RE,
    `${CommonMarkModel.NAMESPACE}.Item`,
    options,
  );
  const foreachBlockResults = await generateRecursiveBlocks(
    modelManager,
    clauseLibrary,
    templateMark,
    data,
    FOREACH_DEFINITION_RE,
    `${CommonMarkModel.NAMESPACE}.Paragraph`,
    options,
  );
  // evaluate all optional blocks (async)
  const optionalBlockResults = await generateOptionalBlocks(
    modelManager,
    clauseLibrary,
    templateMark,
    data,
    ...
    if (
      ...
      // map over an array of items, joining them into a Text node
        ...
        if (variableValues.length === 0) {
          ...
          if (!Array.isArray(arrayData)) {
            ...
            context.text = joinList(
              arrayData.map((arrayItem) => {
                return drafter
                  ? drafter(arrayItem, context.format)
                  : (arrayItem as string);
              }),
              context,
              options,
      ...
      // with the value of the variable from 'data'
        ...
        if (typeof data === "object") {
          ...
          if (variableValues.length === 0) {
            ...
            const type = (ModelUtil as any).isPrimitiveType(context.elementType)
              ? null
              : introspector.getClassDeclaration(context.elementType);

            const drafter = getDrafter(
              type && type.isEnum() ? "String" : context.elementType,
            );

            const draftedValue = drafter
              ? drafter(variableValue, context.format)
              : JSON.stringify(variableValue);

            // 🔥 Vocabulary Integration
            let label: string | undefined;

            if (options?.vocabularyManager && options?.locale && context.name) {
              const fallbackChain = getLocaleFallbackChain(options.locale);

              const classDecl = introspector.getClassDeclaration(
                context.elementType,
              );
              const namespace = classDecl.getNamespace();
              const declarationName = classDecl.getName();

              for (const loc of fallbackChain) {
                const term = options.vocabularyManager.resolveTerm(
                  modelManager,
                  namespace,
                  loc,
                  declarationName,
            ...
            // If label found → prepend label
            context.value = label ? `${label}: ${draftedValue}` : draftedValue;
          ...
          context.value = drafter
            ? drafter(variableValue, context.format)
            : (JSON.stringify(variableValue) as string);
        }
      }
        ...
        if (context.condition) {
          ...
          if (variableValues && variableValues.length) {
            if (variableValues.length === 1) {
              ...
              throw new Error(
                `Multiple values found for path '${path}' in data ${data}.`,
              );
            }
          } else {
      ...
      // only include the children of a clause if its condition is true
        ...
        if (
          ...
          variableValues[0] === undefined ||
          variableValues[0] === null
        ) {
          delete context.nodes;
          stopHere = true;
        } else if (context.condition) {
          checkCode(context.condition);
          const result = !!userCodeResults[
            this.path.join("/")
...
export class TemplateMarkInterpreter {
  ...
  async compileTypeScriptToJavaScript(templateMark: object): Promise<object> {
    ...
    if (clazz !== DOCUMENT_ROOT) {
      throw new Error(
        `JSON is not CommonMark. $class is '${clazz}'. ${JSON.stringify(
          templateMark,
          null,
          2,
        )}`,
    ...
    if (
      !(templateMark as any).nodes ||
      !(templateMark as any).nodes.length ||
      (templateMark as any).nodes.length < 1
    ) {
      throw new Error(
        `CommonMark does not have nodes: ${JSON.stringify(
          templateMark,
          null,
          2,
        )}`,
    ...
    if (!TEMPLATEMARK_ROOT_NODES.includes(firstChildClazz)) {
      throw new Error(
        `First child is not templatemark. $class is '${firstChildClazz}'. ${JSON.stringify(
          templateMark,
          null,
          2,
        )}`,
    ...
    if (!templateConcept) {
      throw new Error(
        `First child is not typed: ${JSON.stringify(templateMark, null, 2)}`,
      );
    }
