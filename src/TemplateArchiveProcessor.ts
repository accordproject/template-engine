...
export class TemplateArchiveProcessor {
  ...
  async draft(
    ...
    const templateKind =
      metadata.getTemplateType() !== 0 ? "clause" : "contract";
    ...
    // Ensure CommonMark Document always has nodes array
  ...
  async trigger(
    ...
    if (logicManager.getLanguage() === "typescript") {
      ...
      const tsFiles: Array<Script> = logicManager
        .getScriptManager()
        .getScriptsForTarget("typescript");
