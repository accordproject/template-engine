...
`);
    ...
    ])('rejects malformed TypeScript trigger result %j', async (triggerResult, errorMessage) => {
        const template = await loadTemplate();
        const templateArchiveProcessor = new TemplateArchiveProcessor(template);
        const stateResponse = await templateArchiveProcessor.init(VALID_DATA);
        jest.spyOn(JavaScriptEvaluator.prototype, 'evalDangerously').mockResolvedValueOnce({ result: triggerResult });

        await expect(templateArchiveProcessor.trigger(VALID_DATA, VALID_REQUEST, stateResponse.state))
            .rejects.toThrow(errorMessage);
    });

    it('rejects malformed TypeScript init results', async () => {
        const template = await loadTemplate();
        const templateArchiveProcessor = new TemplateArchiveProcessor(template);
        jest.spyOn(JavaScriptEvaluator.prototype, 'evalDangerously').mockResolvedValueOnce({ result: {} });

        await expect(templateArchiveProcessor.init(VALID_DATA))
            .rejects.toThrow('Invalid init result: missing state object');
    });
});
